...
class AnimalTopDown(BasePose):
    def __init__(self,
        ...
        self.init_weights(pretrained=pretrained, clip_pretrained=clip_pretrained)
        ...
        #self.CrossModelAttSeq = CrossModelAttSeq(text_dim)
    ...
    def init_weights(self, pretrained=None, clip_pretrained=None):
        if hasattr(self.backbone, 'initialize_weights'):
            self.backbone.initialize_weights()
        else:
            self.backbone._load_state_dict(pretrained=clip_pretrained)
        if self.with_keypoint:
            self.keypoint_head.init_weights()
